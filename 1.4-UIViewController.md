...
## IBAction の設定
MixiViewController の xib 上に配置した Button など View コンポーネントからイベントを取得しましょう。
IBOutlet の設定と同じ操作をして、項目設定画面になったら Action を選んでください。またメソッド名も入力してください。
...
また実装ファイルにも空のメソッドが追加されています。ここに MixiSampleViewController の view を表示させる、非表示にするという処理を加えましょう。
